...
@Component({
  selector: 'app-productstudent',
  template:`
  <!-- breadcrumb -->
  <app-headerstudent></app-headerstudent>
      ...
      <div class="col-lg-4 col-md-4 col-sm-4 col-xs-6" *ngFor="let book of books | filterdata: queryString : 'title'; let i = index">
        ...
        <div class="card ecom-home-catalog-card">
          ...
          <div class="card-body">
            ...
            <div *ngIf="!book.borrow && book.quantity>0" class="btn btn-primary ecom-home-catalog-add-cart" id="{{i}}">
              <span class="ecom-responsive" (click)="borrowBook(i)">Borrow</span>
            </div>&nbsp;&nbsp;
            <!-- todo a verifier -->
          </div>
          <!-- /.card-body -->
  ...
  <!-- /.ecom-modal-container -->
  `,
  styleUrls: ['./productstudent.component.scss']
})
export class ProductstudentComponent implements OnInit {
  ...
  borrowBook(i:number):void{
    this.books[i].borrow=true;
    alert("Successfully borrowed a book");
    //this.books.splice(i,1);
    this._bookService.borrowingBook(this.books[i].id);
  }
